...
- 请求 DeepSeek 时会自动压缩较早对话，只保留最近关键上下文的完整内容。
- 历史附件不会反复把大段文本和图片重新发送；只有当前最新提问的图片会随请求提交。
- 数理和编程模式遇到长推导、长代码任务时会倾向分段输出；回答中断时可点消息下方的“继续回答”按钮接着写。
- 如果请求等待过久，前端会停止等待并给出可继续的中断提示，避免手机端一直卡在无响应状态。
